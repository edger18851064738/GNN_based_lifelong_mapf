...
# 🧠 导入GAT训练解决方案
try:
    from gat_training_solution import SmartGATCoordinator, HeuristicCoordinator
    HAS_SMART_GAT = True
    print("✅ 智能GAT协调器导入成功")
except ImportError:
    HAS_SMART_GAT = False
...
# 🎯 内置简化启发式协调器
class SimpleHeuristicCoordinator:
    """简化的启发式协调器"""
    ...
    def generate_coordination_guidance(self, vehicles_info: List[Dict]) -> List[CoordinationGuidance]:
        ...
        for vehicle_info in vehicles_info:
            ...
            guidance = CoordinationGuidance(
                ...
                adjusted_priority=vehicle_info['priority'] + priority_adj
            )
...
class EnhancedFirstRoundPlanner:
    ...
    def __init__(self, map_file: str, optimization_level: OptimizationLevel = OptimizationLevel.ENHANCED,
        ...
        # 🆕 智能GAT协调系统初始化
        self.enable_gat = enable_gat and (HAS_GAT or HAS_SMART_GAT)
        if self.enable_gat:
            self._initialize_smart_gat_system()
        else:
    ...
    def _initialize_smart_gat_system(self):
        """🆕 初始化智能GAT协调系统"""
        try:
            # 优先使用智能GAT协调器
            if HAS_SMART_GAT:
                self.smart_coordinator = SmartGATCoordinator(
                    enable_training=True,
                    model_path="gat_coordination_model.pth"  # 尝试加载预训练模型
                )
                print("   🧠 智能GAT协调器初始化成功")
                print(f"     - 状态: {'训练GAT' if self.smart_coordinator.use_gat else '启发式规则'}")
                return
            ...
            # 🎯 最后回退到内置启发式协调器
                ...
                self.simple_heuristic = SimpleHeuristicCoordinator()
                print("   🎯 使用内置启发式协调器")
                return
            
        except Exception as e:
            print(f"❌ 智能GAT系统初始化失败: {e}")
            # 最终回退
            self.simple_heuristic = SimpleHeuristicCoordinator()
            print("   🎯 回退到内置启发式协调器")
    ...
    def apply_gat_coordination(self):
        ...
        try:
            ...
            if HAS_SMART_GAT and hasattr(self, 'smart_coordinator'):
                # 🆕 使用智能GAT协调器（包含训练和启发式）
                guidance_list = self.smart_coordinator.generate_coordination_guidance(vehicles_info)
                coordination_time = time.time() - start_time
                
                print(f"   🎯 智能协调完成: {len(guidance_list)}个指导策略, 耗时{coordination_time:.3f}s")
                
                # 如果还没有训练好的模型，可以在这些场景上进行训练
                if not self.smart_coordinator.use_gat:
                    print("   💡 提示: 当前使用启发式方法，可通过更多场景数据训练GAT模型")
                
            elif hasattr(self, 'simple_heuristic'):
                # 🎯 使用内置启发式协调器
                guidance_list = self.simple_heuristic.generate_coordination_guidance(vehicles_info)
                coordination_time = time.time() - start_time
                
                print(f"   🎯 启发式协调完成: {len(guidance_list)}个指导策略, 耗时{coordination_time:.3f}s")
                
            else:
                # 回退到原始GAT方法
                print("   ⚠️ 使用原始GAT方法 (未训练)")
                guidance_list = self._original_gat_coordination(vehicles_info)
                coordination_time = time.time() - start_time
            ...
            # 更新统计
            self.gat_stats['graph_construction_time'] = coordination_time * 0.3  # 估算
            self.gat_stats['inference_time'] = coordination_time * 0.5
            self.gat_stats['decision_parsing_time'] = coordination_time * 0.2
            
            print(f"✅ 智能GAT协调应用成功")
            
        except Exception as e:
            print(f"❌ 智能GAT协调失败: {str(e)}")
            import traceback
            traceback.print_exc()
    
    def _original_gat_coordination(self, vehicles_info: List[Dict]) -> List[CoordinationGuidance]:
        """原始GAT协调方法（未训练）"""
        # 构建图数据
        graph_data = self.graph_builder.build_graph(vehicles_info)
        
        # GAT推理（注意：未训练的GAT输出是随机的）
        ...
        print(f"      ⚠️ 警告: 使用未训练GAT，决策质量可能较差")
        return guidance_list
    ...
    def _plan_single_vehicle_enhanced(self, vehicle: Vehicle, existing_trajectories: List) -> Optional[List[VehicleState]]:
        ...
        try:
            # 🆕 智能GAT指导规划
            if (self.enable_gat and vehicle.gat_guidance and 
                HAS_SMART_GAT and hasattr(self, 'smart_coordinator') and 
                self.smart_coordinator.use_gat):
            ...
            elif self.enable_gat and vehicle.gat_guidance and HAS_GAT:
                # 使用原始GAT集成规划器（但提醒未训练）
                print(f"      ⚠️ 使用未训练GAT指导规划")
                trajectory = self.integrated_planner.plan_single_vehicle(
                    start_state, goal_state, vehicle.vehicle_id, 
                    vehicle.gat_guidance, existing_trajectories
                )
...
def main():
    ...
    # 🆕 GAT训练选项
    if enable_gat:
        train_choice = input("是否在当前场景上训练GAT? (y/N): ").strip().lower()
        enable_training = train_choice in ['y', 'yes']
        print(f"🎓 GAT训练: {'✅ 启用' if enable_training else '❌ 跳过'}")
    else:
    ...
    try:
        ...
        # 🆕 GAT训练（如果启用）
        if enable_training and enable_gat:
            print(f"\n🎓 开始GAT训练过程...")
            
            # 创建训练场景数据
            scenarios_data = []
            for vehicle in planner.vehicles:
                vehicles_info = planner._convert_vehicles_to_gat_format()
                scenarios_data.append({
                    'name': f'training_scenario_{len(scenarios_data)+1}',
                    'vehicles_info': vehicles_info
                })
            
            # 在智能协调器上训练
            if HAS_SMART_GAT and hasattr(planner, 'smart_coordinator'):
                planner.smart_coordinator.train_on_scenarios(scenarios_data)
            else:
                print("   ⚠️ 智能协调器不可用，跳过训练")
        ...
        if success:
            ...
            if enable_gat:
                print(f"\n🧠 GAT系统效果总结:")
                if HAS_SMART_GAT and hasattr(planner, 'smart_coordinator'):
                    if planner.smart_coordinator.use_gat:
                        print("   ✅ 使用训练好的GAT模型")
                    else:
                        print("   🎯 使用启发式规则 (GAT未训练或不可用)")
                elif hasattr(planner, 'simple_heuristic'):
                    print("   🎯 使用内置启发式协调器")
                else:
                    print("   ⚠️ 使用未训练的GAT (效果可能不佳)")
                
                print(f"   📊 最终成功率: {100*planner.successful_plannings/planner.total_vehicles:.1f}%")
        else:
            print("⚠️ 规划成功率较低")
            ...
            if enable_gat:
                print("\n💡 改进建议:")
                print("   1. 尝试收集更多训练数据来训练GAT")
                print("   2. 调整启发式规则参数")
                print("   3. 或暂时禁用GAT使用传统规划")
